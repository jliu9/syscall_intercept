...
# Create error bars representing +SD and -SD
error_bars = np.array([stddevs, stddevs])

ax.bar(labels, means, yerr=error_bars, capsize=5, color='#3E8EDE')
ax.set_title('Absolute Times with Variance')
ax.set_ylabel('Time (ns)')
ax.set_xticklabels(labels, rotation=45, ha='right')
plt.tight_layout()
plt.savefig(f'{dir_name}/absolute_times.png')  # Save the first plot

# Normalized values plot with box plot
fig, ax = plt.subplots(figsize=(10, 6))
baseline_mean = stats['Baseline'][0]
normalized_means = [m / baseline_mean for m in means]
normalized_data = [np.array(times) / baseline_mean for times in stats.values()]

# Bar plot over the box plot
bars = ax.bar(labels, normalized_means, alpha=0.5, color='#3E8EDE')
...
ax.set_xticklabels(labels, rotation=45, ha='right')
plt.tight_layout()
plt.savefig(f'{dir_name}/normalized_times.png')  # Save the second plot
